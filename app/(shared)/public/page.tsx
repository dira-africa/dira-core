...
const localT = {
  en: {
    ...
    midnightTitle: "Midnight Blockchain Verification Panel",
    midnightSub: "Weekly Merkle-tree anchoring hashes recorded on the Midnight network",
    midnightWeek: "Week No",
    midnightBatch: "Batch Merkle Root",
    midnightTx: "Midnight Tx Hash",
  ...
  sw: {
    ...
    midnightTitle: "Uthibitishaji wa Midnight Blockchain",
    midnightSub: "Mihuri ya kila wiki ya Merkle-tree iliyohifadhiwa kwenye mtandao wa Midnight",
    midnightWeek: "Wiki",
    midnightBatch: "Chapa ya Merkle Root",
    midnightTx: "Miamala ya Midnight",
...
export default function PublicDashboard() {
  ...
  const [anchors, setAnchors] = useState<MidnightAnchor[]>([]);
  ...
  const fetchTelemetry = async () => {
    try {
      const [statsRes, econRes, feedRes, qualRes, anchRes] = await Promise.all([
        ...
        fetch(`${API_URL}/public/midnight-anchors`)
      ]);
  ...
  return (
    <main className="flex-1 w-full min-h-screen bg-gradient-to-b from-[#0a0a23] via-[#051614] to-[#040f0c] text-white font-sans overflow-x-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Section 4: Midnight ZK Verification */}
                ...
                {anchors.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="p-8 text-center text-white/30 text-xs">
                      No anchors uploaded on the Midnight ledger.
                    </td>
                  </tr>
                ) : (
                  anchors.map((anch, index) => (
                    <tr key={index} className="hover:bg-white/[0.01] transition-all">
                      ...
                      <td className="p-4 break-all font-mono">
                        <a
                          href={`https://explorer.midnight.network/transaction/${anch.midnightTxHash}`}
                          target="_blank"
